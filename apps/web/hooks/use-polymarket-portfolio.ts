...
export function getPositionValue(position: any) {
  ...
  const currentPrice = numericPortfolioValue(position?.curPrice, position?.cur_price, position?.current_price, position?.price)
...
export function getPositionPnl(position: any) {
  const size = numericPortfolioValue(position?.size, position?.position_size)
  const avgPrice = numericPortfolioValue(position?.avgPrice, position?.avg_entry_price, position?.entry_price)
  const currentPrice = numericPortfolioValue(position?.curPrice, position?.cur_price, position?.current_price, position?.price)
  const fallbackPnl = size && currentPrice && avgPrice ? size * (currentPrice - avgPrice) : 0
  return numericPortfolioValue(
    position?.cashPnl,
    position?.cash_pnl,
...
export function getPositionPnlPercent(position: any) {
  ...
  const initialValue = numericPortfolioValue(position?.initialValue, position?.initial_value)
  const fallbackPercent = initialValue ? (pnl / initialValue) * 100 : 0
  return numericPortfolioValue(position?.percentPnl, position?.percent_pnl, position?.pnlPercent, position?.pnl_percent, fallbackPercent)
}
