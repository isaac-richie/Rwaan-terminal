...
const TIERS = [
  {
    ...
    max: 499,
    icon: Star,
    perks: ["Basic market access", "Public leaderboard rank"],
  },
  {
    name: "Analyst",
    ...
    max: 1999,
    icon: BarChart3,
    perks: ["Early market signals", "Discounted analysis unlocks", "Analyst badge"],
  },
  {
    name: "Strategist",
    ...
    max: 7499,
    icon: TrendingUp,
    perks: ["Priority order routing", "Exclusive intelligence feeds", "Season rewards multiplier"],
  },
  {
    name: "Oracle",
...
const EARNING_METHODS = [
  ...
  },
  {
    ...
    detail: "Founding traders who trade before Season 1 launch receive a multiplier.",
    pts: "2× boost",
...
function tierForPoints(points: number) {
  return TIERS.reduce((current, tier) => (points >= tier.min ? tier : current), TIERS[0]);
}

function nextTierForPoints(points: number) {
  return TIERS.find((tier) => points < tier.min) ?? null;
}
  ...
  return (
    <div
      ...
      style={{
        ...
        background: `${tier.bg}`,
        ...(current ? { outlineColor: tier.color } : {}),
      }}
    >
      {current && (
        <div
          className="absolute right-3 top-3 rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-widest"
...
export default function PointsPage() {
  ...
  const progressPct = nextTier
    ? Math.max(0, Math.min(100, ((totalPoints - currentTier.min) / (nextTier.min - currentTier.min)) * 100))
    : 100;
  const CurrentTierIcon = currentTier.icon;
  const completedQuests = quests.filter((quest) => quest.completed).length;
  const resetLabel = questResetAt
    ? new Date(questResetAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })
    : "--:--";
      ...
      <main className="relative z-10 mx-auto flex w-full max-w-7xl flex-1 flex-col px-4 pb-20 pt-24 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 grid gap-4 lg:grid-cols-12">
          <div className="surface-card overflow-hidden rounded-2xl p-6 lg:col-span-4">
            ...
            <div className="mt-3 flex items-center gap-2">
              <div
                className="flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest"
            ...
            <div className="mt-6">
              <div className="flex items-center justify-between text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">
                ...
                <span>{nextTier ? `${totalPoints.toLocaleString()} / ${progressMax.toLocaleString()}` : `${totalPoints.toLocaleString()}+`}</span>
              </div>
              <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[oklch(0.18_0.014_255)]">
                <div className="h-full rounded-full bg-[oklch(0.68_0.18_155)]" style={{ width: `${progressPct}%` }} />
              </div>
            </div>
          ...
          <div className="flex flex-col gap-4 lg:col-span-8">
            <div className="surface-card rounded-2xl p-5">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-[oklch(0.78_0.16_82/0.25)] bg-[oklch(0.78_0.16_82/0.08)]">
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-4">
              {[
                { label: "Trades tracked", value: summary?.trades?.toLocaleString() ?? "—", icon: Users },
                { label: "Routed volume", value: summary ? `$${summary.volumeUsd.toFixed(0)}` : "—", icon: BarChart3 },
                { label: "Unlocks", value: summary?.premiumUnlocks?.toLocaleString() ?? "—", icon: Sparkles },
              ].map((stat) => {
                const Icon = stat.icon;
                return (
                  <div key={stat.label} className="surface-card rounded-2xl p-3 sm:p-4">
                    <Icon className="h-4 w-4 text-[oklch(0.78_0.16_82)]" />
                    <div className="mt-2 sm:mt-3 font-mono text-xl sm:text-2xl font-bold text-foreground">{stat.value}</div>
                    <div className="mt-1 text-[9px] sm:text-[10px] uppercase tracking-[0.12em] sm:tracking-[0.14em] text-muted-foreground">{stat.label}</div>
                  </div>
                );
              })}
            </div>
            ...
            {quests.slice(0, 6).map((quest) => {
              ...
              return (
                <div
                  key={quest.id}
                  className={cn(
                    "surface-card rounded-2xl border p-5 transition-colors",
                    quest.completed
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="rounded-md border border-[oklch(0.78_0.16_82/0.20)] bg-[oklch(0.78_0.16_82/0.08)] px-2 py-1 font-mono text-[10px] font-bold text-[oklch(0.82_0.16_82)]">
                      {quest.reward}
                    </span>
                  </div>
                  <div className="mt-4 text-sm font-bold text-foreground">{quest.title}</div>
                  <p className="mt-1.5 min-h-10 text-xs leading-5 text-muted-foreground">{quest.detail}</p>
                  <div className="mt-4">
                    <div className="flex items-center justify-between text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">
                      <span>{quest.completed ? "Complete" : "Progress"}</span>
                    ...
                    <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[oklch(0.18_0.014_255)]">
                      <div
                        className={cn(
                          ...
                          quest.completed ? "bg-[oklch(0.68_0.18_155)]" : "bg-[oklch(0.78_0.16_82)]"
                        )}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
        ...
        <section className="mt-12">
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <h2 className="mt-1.5 text-2xl font-bold text-foreground">Four tiers. Four levels of edge.</h2>
            </div>
          </div>
        ...
        <section className="mt-12">
          <div className="mb-6">
            ...
            <h2 className="mt-1.5 text-2xl font-bold text-foreground">Every action compounds.</h2>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              return (
                ...
                >
                  <div
                    className="flex h-10 w-10 items-center justify-center rounded-xl border"
                    style={{
                      background: `${method.color.replace(")", "/0.08)").replace("oklch(", "oklch(")}`,
                      borderColor: `${method.color.replace(")", "/0.25)").replace("oklch(", "oklch(")}`,
                    }}
                  >
                    <Icon className="h-4 w-4" style={{ color: method.color }} />
                  </div>
                  <div className="mt-4 text-sm font-semibold text-foreground">{method.title}</div>
                  <p className="mt-1.5 text-xs leading-5 text-muted-foreground">{method.detail}</p>
                  <div
        ...
        <section className="mt-12">
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <h2 className="mt-1.5 text-2xl font-bold text-foreground">Top traders this season.</h2>
            </div>
            <button
              onClick={() => setShowAll((v) => !v)}
              className="flex items-center gap-1.5 rounded-xl border border-[oklch(0.22_0.015_255)] bg-[oklch(0.15_0.014_255)] px-3 py-1.5 text-[11px] font-semibold text-muted-foreground transition-colors hover:border-[oklch(0.78_0.16_82/0.4)] hover:text-foreground"
            >
          ...
          <div className="surface-card overflow-hidden rounded-2xl">
            {/* Header */}
            <div className="grid grid-cols-[1.5rem_1fr_auto_auto] sm:grid-cols-[2rem_1fr_auto_auto] items-center gap-2 sm:gap-4 border-b border-[oklch(0.18_0.014_255)] px-3 sm:px-5 py-3 text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">
              <span>#</span>
              <span>Trader</span>
              <span>Tier</span>
              <span>Points</span>
            ...
            {!leaderboardLoading && leaderboard && leaderboard.length > 0 && (showAll ? leaderboard : leaderboard.slice(0, 3)).map((row) => {
              ...
              return (
                <div
                  key={row.rank}
                  className="grid grid-cols-[1.5rem_1fr_auto_auto] sm:grid-cols-[2rem_1fr_auto_auto] items-center gap-2 sm:gap-4 border-b border-[oklch(0.14_0.012_260)] px-3 sm:px-5 py-4 last:border-0 hover:bg-[oklch(0.14_0.012_260/0.60)] transition-colors"
                >
                  <span
                    className={cn(
                      "font-mono text-sm font-bold",
                      row.rank === 1 && "text-[oklch(0.72_0.22_45)]",
                      row.rank === 2 && "text-[oklch(0.70_0.04_255)]",
                      row.rank === 3 && "text-[oklch(0.62_0.10_50)]",
                  ...
                  <span
                    ...
                    style={{ borderColor: tierData.border, background: tierData.bg, color: tierData.color }}
                  >
                    {row.tier}
                  </span>
        ...
        <section className="mt-10 rounded-2xl border border-[oklch(0.78_0.16_82/0.20)] bg-[oklch(0.78_0.16_82/0.05)] p-6 sm:p-8">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a
              href="/"
              className="inline-flex h-11 shrink-0 items-center gap-2 rounded-xl bg-[oklch(0.78_0.16_82)] px-6 text-sm font-bold text-[oklch(0.10_0.012_260)] transition-colors hover:bg-[oklch(0.83_0.16_82)]"
            >
